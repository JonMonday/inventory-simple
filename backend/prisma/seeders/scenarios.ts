...
import { log, assert, asCode } from './seed.helpers';

export async function createScenarios(prisma: PrismaClient) {
    log('SCENARIOS', 'Building operational workflow scenarios (S1-S8)...');
    ...
    const getStatus = (c: string) => statuses.find(s => s.code === c)!;

    const eventTypes = await prisma.requestEventType.findMany();
    const getEvent = (c: string) => eventTypes.find(e => e.code === c)!;

    const moveTypes = await prisma.ledgerMovementType.findMany();
    const getMove = (c: string) => moveTypes.find(m => m.code === c)!;

    const pRoles = await prisma.participantRoleType.findMany();
    const getPRole = (c: string) => pRoles.find(r => r.code === c)!;
    ...
    // S2: Submit -> Rejected -> Edited -> Resubmitted
    ...
    // S5: Reserve Success -> Cancel (Stock RESTORE)
    ...
    // S6: Reserve -> Issue (Ledger + Stock Decr)
    ...
    log('SCENARIOS', `Built ${scenarioCount} operational scenarios.`);
    assert(scenarioCount >= 7, 'Expected mandatory scenarios build failure');
}
