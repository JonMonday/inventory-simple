...
import { upsertByCode, log, assert, permsMap } from './seed.helpers';

export async function seedStatic(prisma: PrismaClient) {
    log('STATIC', 'Starting system foundation seed...');

    // 1. Permissions (The 128-route Perimeter)
    // 1. Permissions (from shared/permissions.json)
    log('STATIC', 'Seeding permissions from shared JSON...');
    ...
    if (!permsData.permissions) throw new Error('Invalid permissions.json structure');

    const pRecords: Record<string, string> = {};
    const permissions: string[] = []; // Re-introduce variable for legacy usage
    ...
    // 2. Roles
    log('STATIC', 'Seeding system roles...');
    const roles = [
    ...
    for (const r of roles) {
        log('STATIC', `   -> Seeding role: ${r.code}`);
    ...
    // 3. Workflow Meta (Statuses & Stages)
    log('STATIC', 'Seeding workflow metadata...');
    const statuses = [
    ...
    for (const s of statuses) await upsertByCode(prisma.requestStatus, s.code, s);
    ...
    for (const st of stages) await upsertByCode(prisma.requestStageType, st.code, st);
    ...
    for (const et of eventTypes) await upsertByCode(prisma.requestEventType, et, { label: et.replace(/_/g, ' ') });

    const moveTypes = ['RECEIVE', 'RETURN', 'TRANSFER', 'ADJUST', 'ISSUE', 'REVERSE'];
    for (const mt of moveTypes) await upsertByCode(prisma.ledgerMovementType, mt, { label: mt.charAt(0) + mt.slice(1).toLowerCase() });

    const itemStats = ['ACTIVE', 'INACTIVE', 'DISCONTINUED'];
    for (const is of itemStats) await upsertByCode(prisma.itemStatus, is, { label: is.charAt(0) + is.slice(1).toLowerCase() });

    const commentTypes = [
        'GENERAL', 'REJECTION_COMMENT', 'APPROVAL_NOTE', 'REVIEW_NOTE',
        'CANCELLATION_COMMENT', 'SYSTEM_NOTE'
    ];
    for (const ct of commentTypes) await upsertByCode(prisma.commentType, ct, { label: ct.replace(/_/g, ' ') });

    const participantRoles = ['REQUESTER', 'REVIEWER', 'APPROVER', 'STOREKEEPER', 'ADMIN'];
    for (const pr of participantRoles) await upsertByCode(prisma.participantRoleType, pr, { label: pr.charAt(0) + pr.slice(1).toLowerCase() });

    const stakeStats = ['DRAFT', 'STARTED', 'SUBMITTED', 'APPROVED', 'APPLIED', 'CANCELLED'];
    for (const ss of stakeStats) await upsertByCode(prisma.stocktakeStatus, ss, { label: ss.charAt(0) + ss.slice(1).toLowerCase(), sortOrder: stakeStats.indexOf(ss) + 1 });

    // 5. System User & Default Template
    log('STATIC', 'Seeding system user and default template...');
    const systemEmail = 'system@gra.local';
    // Note: System user needs org links. We'll check if any exist or create placeholders.
    let branch = await prisma.branch.findFirst();
    if (!branch) branch = await upsertByCode(prisma.branch, 'SYS', { name: 'System Entities' });
    let dept = await prisma.department.findFirst();
    if (!dept) dept = await upsertByCode(prisma.department, 'SYS', { name: 'System Operations', branchId: branch!.id });
    let unit = await prisma.unit.findFirst();
    if (!unit) unit = await upsertByCode(prisma.unit, 'SYS_UNIT', { name: 'System Unit', departmentId: dept!.id });
    let jobRole = await prisma.jobRole.findFirst();
    if (!jobRole) jobRole = await upsertByCode(prisma.jobRole, 'SYS_ROLE', { name: 'System Service', unitId: unit!.id });
    ...
    const systemUser = await prisma.user.upsert({
        ...
        update: {
            isActive: true,
            passwordHash: systemPasswordHash, // Update password hash on every seed
    ...
    for (let i = 0; i < stdSteps.length; i++) {
        ...
        await prisma.templateWorkflowStep.upsert({
            ...
            update: { stageTypeId: stageType.id, assignmentMode: s.mode as any, roleKey: s.role },
            create: {
                templateId: template.id, stageTypeId: stageType.id, stepOrder: i + 1,
                isRequired: true, assignmentMode: s.mode as any, roleKey: s.role
            }
        });
    }

    // SPECIAL: DT Assessment Template (Desktop / Tablet)
    ...
    const dtStepsData = [
        { code: 'UNIT_REVIEW', role: 'unit_head', mode: 'AUTO_POOL' },
        { code: 'UNIT_APPROVAL', role: 'unit_head', mode: 'MANUAL_FROM_POOL' }, // Manual selection from pool
        { code: 'PROC_REVIEW', role: 'procurement_officer', mode: 'AUTO_POOL' },
        { code: 'FULFILLMENT', role: 'storekeeper', mode: 'AUTO_POOL' },
        { code: 'CONFIRMATION', role: 'requester', mode: 'AUTO_POOL' },
    ];
    ...
    // 6. Final Static Validation
    log('STATIC', 'Finalizing static validation...');
    assert(await prisma.requestTemplate.count({ where: { id: 'default-request-template-static' } }) === 1, 'Default template missing');
    assert(await prisma.templateWorkflowStep.count({ where: { templateId: template.id } }) === 6, 'Template steps count mismatch');
    log('STATIC', 'System foundation seeded successfully.');
}
