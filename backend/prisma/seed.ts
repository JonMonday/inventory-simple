...
async function main() {
    ...
    // Create locations first (they're created later in the file, but we need to reference them)
    // We'll update the admin user after locations are created
    const adminUser = await prisma.user.upsert({
        ...
        create: {
            ...
            isActive: true,
            // locationId and departmentId will be set after locations are created
        },
    });
    ...
    // Update admin user with location and department
    ...
    // Update demo user with location and department
    await prisma.user.update({
        where: { email: 'user@example.com' },
        data: {
            locationId: mainStore.id,
