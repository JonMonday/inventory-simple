...
const adapter = new adapter_better_sqlite3_1.PrismaBetterSqlite3({ url: dbUrl.replace('file:', '') });
...
async function main() {
    ...
    for (const dist of distributions) {
        ...
        for (let c = 0; c < dist.count; c++) {
            ...
            const storeLoc = branch.locations.find((l) => l.type === 'STORE');
            const deptLocs = branch.locations.filter((l) => l.type === 'DEPARTMENT');
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        for (const b of branches) {
            const store = b.locations.find((l) => l.type === 'STORE');
